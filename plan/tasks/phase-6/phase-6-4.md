...
Dynamic Graph 생성을 실행하는 스크립트를 구현합니다. 티커별로 Dynamic Graph를 생성하고, Embedding 생성 옵션을 지원하며 JSON 파일로 저장합니다.

### 파일 경로
**파일**: `scripts/05_generate_dynamic_graph.py`

### Phase 전체 목표 기여
- Dynamic Graph 생성 프로세스 자동화
- 여러 티커에 대한 일괄 처리 지원
- Embedding 생성 옵션 제공
- JSON 파일로 결과 저장

### 입력 데이터
- **커맨드라인 인자**: `--ticker` 옵션으로 티커 심볼 지정, `--with-embedding` 옵션
- **Extracted 데이터**: `data/extracted/{ticker}/` 폴더의 JSON 파일들
- **Static Graph**: `data/graph/{TICKER}_static_graph.json` 파일
...
1. **커맨드라인 인자 파싱**
   ...
   - `--with-embedding` 옵션으로 Embedding 생성 여부 지정

2. **Dynamic Graph 생성**
   - 각 티커에 대해 `generate_dynamic_graph` 함수 호출
   - Extracted 디렉토리 및 Static Graph 확인

3. **Embedding 생성**
   - `--with-embedding` 옵션이 있으면 Embedding 생성
   - Risk, Opportunity, Event, Technology 노드에 Embedding 추가

4. **JSON 파일 저장**
   - 생성된 Dynamic Graph를 JSON 파일로 저장
   - `data/graph/{TICKER}_dynamic_graph.json` 경로에 저장
...
def main():
    ...
    for ticker in tickers:
        ...
        # Static Graph 확인
        static_graph_file = Path(f"data/graph/{ticker}_static_graph.json")
        if not static_graph_file.exists():
            logger.error(f"Static graph not found: {static_graph_file}")
            continue
        
        # Static Graph 로드
        with open(static_graph_file, 'r', encoding='utf-8') as f:
        ...
        try:
            ...
            logger.info(
                f"📊 Summary: "
                f"Documents={len(nodes['Document'])}, "
                f"Sections={len(nodes['Section'])}, "
                f"Risks={len(nodes['Risk'])}, "
                f"Opportunities={len(nodes['Opportunity'])}, "
                f"Events={len(nodes['Event'])}, "
                f"Technologies={len(nodes['Technology'])}, "
                f"Links={len(dynamic_graph['links'])}"
            )
...
# 여러 티커 처리
python scripts/05_generate_dynamic_graph.py --ticker AAPL TSLA NVDA --with-embedding
```

### 에러 핸들링
- Extracted 디렉토리 없음: 에러 로그 및 다음 티커로 진행
- Static Graph 없음: 에러 로그 및 다음 티커로 진행
- Dynamic Graph 생성 실패: 에러 로그 및 다음 티커로 진행
...
### 실행 프로세스
1. **커맨드라인 인자 파싱**
   - `argparse`를 사용하여 `--ticker` 및 `--with-embedding` 옵션 파싱
   - 티커 심볼을 대문자로 변환

2. **각 티커 처리**
   - Extracted 디렉토리 존재 확인
   - Static Graph 파일 존재 확인 및 로드
   - Extracted 파일 목록 수집
   - `generate_dynamic_graph` 함수 호출
   - 생성된 Dynamic Graph를 JSON 파일로 저장
   - 결과 요약 출력
...
- JSON 파일 구조 확인
- 노드 및 링크 수 확인
- Embedding 필드 확인 (--with-embedding 옵션 사용 시)

## ⚠️ 주의사항

- Extracted 데이터와 Static Graph가 있어야 Dynamic Graph 생성 가능
- Embedding 생성 시 API 비용 발생
