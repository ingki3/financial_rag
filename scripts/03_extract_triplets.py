#!/usr/bin/env python
"""Knowledge Triplet 추출 스크립트 (Gemini 3 Flash 사용)"""
import os
import sys
import json
import logging
...
def parse_args() -> argparse.Namespace:
    p = argparse.ArgumentParser(description="Knowledge Triplet extraction batch runner.")
    p.add_argument(
        "--mode",
        choices=["section", "filing"],
        default="section",
        help="section: save one JSON per section (accession_section.json). filing: save one JSON per parsed filing (accession.json) combining sections.",
    )
    return p.parse_args()
...
def main():
    ...
    print("=" * 60)
    print("Knowledge Triplet Extractor")
    ...
    batch_extractor = TripletExtractorBatch(
        ...
        combine_sections_per_filing=(args.mode == "filing"),
    )
